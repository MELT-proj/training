...
**`config/train/ABL-*.yaml` does NOT travel.** It is gitignored (generated by
`infra/build_campaign_config.py`), and `--dirty` mode feeds `.gitignore` to
rsync as a filter, so an edited or regenerated ablation config stays on your
laptop while MN5 keeps whatever it had — silently. Regenerate it **on MN5**,
which is where the data is anyway:
...
The *template* is tracked, so it reaches MN5 by the default (git) mode but not
by `--dirty`. Check `grep -c fleurs config/train/ABL-*.yaml` on MN5 if you are
unsure whether you are looking at a stale render.
