...
from transformers.modeling_outputs import BaseModelOutput, CausalLMOutputWithPast, Seq2SeqLMOutput
...
class SpeechLMPreAdapter(nn.Module):
    def __init__(self, config):
        ...
        self.proj_layer_norm = nn.LayerNorm(config.encoder.output_hidden_size, eps=config.encoder.layer_norm_eps)
        self.layers = nn.ModuleList(
            Wav2Vec2BertAdapterLayer(config.encoder) for _ in range(config.num_pre_adapter_layers)
        )
...
class SpeechLMForConditionalGeneration(SpeechLMPreTrainedModel):
    ...
    def __init__(
        ...
        if config is None:
            config = SpeechLMForConditionalGeneration.from_encoder_decoder_configs(encoder.config, decoder.config)
        else:
    ...
    def from_encoder_decoder_pretrained(
        ...
        **kwargs,
        ...
        config = SpeechLMConfig.from_encoder_decoder_configs(encoder.config, decoder.config, **kwargs)
        ...
        **kwargs,
        ...
        if not use_cache or (use_cache and past_key_values is None):
            ...
            if self.config.add_pre_adapter:
                audio_input_features = self.pre_adapter(audio_input_features, attention_mask=audio_attention_mask)
