...
def emit_nemo_group_yaml(sources: list[dict], out: Path,
    ...
    lines = [
        ...
        "# NeMo/lhotse mux the two levels, so the sampling probability of a corpus",
        "# is the product p_l * p_c. Do not pre-multiply them.",
        "#",
        "# Do NOT set `reweight_temperature` alongside this file. alpha/beta are",
        "# already baked into the weights below; NeMo would raise them to a further",
        "# power. Both levels already sum to 1, so NeMo's normalisation is a no-op.",
        "#",
        f"# {len(sources)} corpora, {len(by_lang)} language entries, {total_hours:,.1f} h total.",
        "input_cfg:",
    ]
    ...
    for lang in sorted(by_lang, key=lambda l: -hours_of(l)):
        ...
        p_l = members[0]["p_l"]
        lines.append(f"  # {lang}: {hours_of(lang):,.1f} h across {len(members)} corpora")
        lines.append("  - type: group")
        lines.append(f"    weight: {p_l:.8f}")
        ...
        for s in members:
            ...
            lines.append(f"        shar_path: {s['raw_path']}")
            lines.append(f"        weight: {s['p_c']:.8f}")
            if s["tags"]:
                lines.append("        tags:")
...
def main() -> None:
    ...
    p.add_argument("--emit-nemo", type=Path, default=None,
                   help="Write a nested NeMo `type: group` input_cfg (p_l on the "
    ...
    if args.emit_nemo:
        ...
        print(f"Wrote {args.emit_nemo} (NeMo group schema)")
