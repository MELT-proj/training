"""Convert Multilingual LibriSpeech (MLS) from HuggingFace to Lhotse Shar format.

Dataset:
  - HF id: facebook/multilingual_librispeech
  - Multilingual: yes (configs correspond to languages, e.g. "german")

Output directory structure:
  {LHOTSE_DATA_SHAR_ROOT}/multilingual_librispeech/{lang}/{split}/

Notes:
- MLS configs are languages. We treat the HF config name as the language key.
- Some splits include extra subsets like "9_hours" and "1_hours".
"""
...
def _get_dataset_config_names() -> list[str]:
    """Best-effort discovery of HF config names."""
    try:
...
def _pick_text(item: dict) -> str:
    # MLS has used both `text` and `transcript` in different dataset versions.
...
def convert_one(
    ...
    *,
    ...
    if use_batched:
        ...
        # If your config uses `text` instead, prefer streaming mode.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert Multilingual LibriSpeech to Lhotse SHAR archives")

    parser.add_argument(
        "--configs",
        nargs="+",
        default=None,
        help="HF config names (languages) to convert, e.g. german french. If omitted, we discover configs.",
    )
    parser.add_argument(
        "--splits",
        nargs="+",
        default=["train", "dev", "test", "9_hours", "1_hours"],
        help="Splits to convert (default includes train/dev/test + 9_hours/1_hours).",
    )
...
def main() -> None:
    ...
    configs = list(args.configs) if args.configs else _get_dataset_config_names()
    if not configs:
