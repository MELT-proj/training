...
#   that is the point of picking this encoder over Whisper (pinned at 30 s by its
#   architecture). MELTAudioEncoder rejects a frame-sized value (the 1500 the
#   other arms use) rather than silently slicing every clip into 94 ms fragments.
#
#   Headroom on 2 x H100, measured: at this 60 s setting the max_duration warmup
#   pass peaks at 29.5 GB of 80 (job 329096). A 90 s cap was also tried and fits
#   -- 23.4 GB, 30 steps clean (job 329301) -- so the 60 s cap is not a memory
#   ceiling, it is a deliberate choice.
#
