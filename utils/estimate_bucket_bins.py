...
Usage:
    python utils/estimate_bucket_bins.py --config-file config/train/asr.yaml
    python utils/estimate_bucket_bins.py --config-file config/train/asr.yaml --train-buckets 30 --val-buckets 20
...
def load_cutset_from_config(
    ...
    Args:
        input_cfg: List of data source configurations.
        min_duration: Minimum cut duration to include.
        max_duration: Maximum cut duration to include.
    ...
    for source_cfg in input_cfg:
        ...
        if source_type == "lhotse_shar":
            ...
            durations = get_durations_from_shar(
...
def estimate_bins_from_durations(
    ...
    if num_buckets > len(durations):
        raise ValueError(
            f"Number of buckets ({num_buckets}) must be <= number of cuts ({len(durations)})"
        )
...
Examples:
    ...
    python utils/estimate_bucket_bins.py --config-file config/train/asr.yaml --train-only
        """,
    )
    parser.add_argument(
        "--config-file",
    ...
    # Load config
    print(f"\nLoading config from: {args.config_file}")
    ...
    if not args.val_only:
        ...
        if train_input_cfg:
            ...
            if train_durations:
                ...
                train_bins = estimate_bins_from_durations(
                    train_durations, args.train_buckets
                )
    ...
    if not args.train_only:
        ...
        if val_input_cfg:
            ...
            if val_durations:
                ...
                val_bins = estimate_bins_from_durations(
                    val_durations, args.val_buckets
                )
